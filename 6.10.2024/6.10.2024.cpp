...
{
	cout << "Student fullname: " << fio.name << " " << fio.secondname << " " << fio.patronymic << endl;
}
...
void print_student2(Student student) {
	...
	cout << "Student " << student.fullName.name << " " << student.fullName.secondname << " " << student.fullName.patronymic << " is " << finalYear << " year(s) "
...
void studentsSurnamesSort(Student students[], int size) {
	for (int i = 0; i < size; i++) {
		...
		for (int j = 0; j < size - 1; j++) {
			if (int(students[j].fullName.secondname[0]) > int(students[j + 1].fullName.secondname[0]))
			{
